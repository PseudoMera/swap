import type { CanopyKeyfile, EncryptedCanopyKeyfile, CanopyKeyfileCollection, KeyfileFormat } from '@/types/wallet';
 ...
 */
export function validateKeyfileFormat(jsonData: unknown): KeyfileValidationResult {
  const errors: string[] = [];
  ...
  if (format === 'encrypted') {
    ...
    const result = validateEncryptedKeyfileFormat(jsonData as CanopyKeyfileCollection);
...
function validateSingleKeyfile(keyfile: unknown, index: number): string[] {
  ...
  if (kf.PublicKey && typeof kf.PublicKey === 'string') {
    ...
    if (!hexRegex.test(publicKey)) {
      errors.push(`${prefix} Public key must contain only hexadecimal characters`);
  ...
  if (kf.PrivateKey && typeof kf.PrivateKey === 'string') {
    ...
    if (!hexRegex.test(privateKey)) {
      errors.push(`${prefix} Private key must contain only hexadecimal characters`);
...
function detectKeyfileFormat(jsonData: unknown): KeyfileFormat {
  ...
  if (firstKey && typeof obj[firstKey] === 'object' && obj[firstKey] !== null) {
    ...
    if ('keyAddress' in firstEntry && 'encrypted' in firstEntry && 'publicKey' in firstEntry) {
      return 'encrypted';
    }
  }
...
function validateEncryptedKeyfileFormat(collection: CanopyKeyfileCollection): {
  ...
  entries.forEach(([nickname, encryptedKeyfile], index) => {
    ...
    const requiredFields = ['publicKey', 'salt', 'encrypted', 'keyAddress', 'keyNickname'];
    
    for (const field of requiredFields) {
      if (!(field in encryptedKeyfile) || typeof encryptedKeyfile[field as keyof EncryptedCanopyKeyfile] !== 'string') {
        errors.push(`${prefix} Missing or invalid field '${field}'`);
      }
    }
    
    // Validate keyAddress format
    if (encryptedKeyfile.keyAddress && typeof encryptedKeyfile.keyAddress === 'string') {
      if (!isValidAddress(encryptedKeyfile.keyAddress)) {
        errors.push(`${prefix} Invalid keyAddress format: ${encryptedKeyfile.keyAddress}`);
      }
    }
    ...
    hexFields.forEach(field => {
      ...
      if (value && typeof value === 'string') {
        ...
        if (!hexRegex.test(value)) {
          errors.push(`${prefix} Field '${field}' must contain only hexadecimal characters`);
    ...
    if (encryptedKeyfile.keyAddress && isValidAddress(encryptedKeyfile.keyAddress)) {
      accounts.push({
...
function isValidAddress(address: string): boolean {
  ...
  return hexWithPrefixRegex.test(address) || hexWithoutPrefixRegex.test(address);
}

/**
 * Get human-readable error message for UI display
 */
export function getValidationErrorMessage(result: KeyfileValidationResult): string {
