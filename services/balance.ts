...
export async function fetchUserBalanceFromChain(chainId: string, payload: AccountQueryPayload): Promise<AccountResponse> {
  ...
  if (!response.ok) throw new Error(`Failed to fetch balance from chain: ${chainId}`);
  return response.json();
}

// Committee-aware balance fetching
export async function fetchUserBalanceByCommittee(committee: number, payload: AccountQueryPayload): Promise<AccountResponse> {
  ...
  if (!response.ok) throw new Error(`Failed to fetch balance for committee: ${committee}`);
  return response.json();
}